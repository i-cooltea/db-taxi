...
-- 修改1：BOOLEAN → TINYINT(1)，FALSE → 0（兼容低版本MySQL，保持布尔逻辑）
...
-- 修改2：BOOLEAN → TINYINT(1)，TRUE → 1（保持业务启用默认逻辑）
enabled TINYINT(1) DEFAULT 1,
-- 修改3：JSON → TEXT（兼容MySQL 5.7以下无原生JSON支持的版本，存储JSON格式字符串）
options TEXT,
created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
sync_mode ENUM('full', 'incremental') NOT NULL DEFAULT 'full',
-- 修改4：BOOLEAN → TINYINT(1)，TRUE → 1（保持表映射启用默认逻辑）
enabled TINYINT(1) DEFAULT 1,
where_clause TEXT,
created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
-- 修改5：JSON → TEXT（兼容低版本MySQL，存储增量同步检查点JSON数据）
